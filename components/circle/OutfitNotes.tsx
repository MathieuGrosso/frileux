...
export function OutfitNotes({ outfitId }: Props) {
  ...
  return (
    <View className="pt-6">
      <Text
        className="font-body-semibold text-ink-300 text-[10px] mb-4"
        style={{ letterSpacing: 1.5 }}
      >
        NOTES DU CERCLE ({notes.length})
      </Text>
      {notes.length === 0 && (
        <Text className="font-body text-ink-300 text-[13px] mb-4">
          ...
          <View key={n.id} className="flex-row items-start gap-2.5 mb-4">
            ...
            <View className="flex-1">
              <View className="flex-row items-baseline gap-2">
                <Text className="font-body-medium text-ink-900 text-[12px]">
                  {n.profile?.username ?? "—"}
                </Text>
              ...
              <Text className="font-body text-ink-700 text-[13px] leading-[18px] mt-0.5">
                {n.body}
              </Text>
            </View>
            {isMine && (
              <Pressable onPress={() => remove(n.id)} className="active:opacity-50 pt-0.5">
                <Text
                  className="font-body-semibold text-ink-300 text-[9px]"
      ...
      <Text className="font-body text-ink-300 text-[9px] mt-1 text-right">
